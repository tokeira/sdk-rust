    ...
    mod sdk_helpers {
        ...
        use crate::protos::{
            ...
            temporal::api::{
                ...
                enums::v1::{
                    ApplicationErrorCategory, TimeoutType, VersioningBehavior,
        ...
        impl WorkflowActivationCompletion {
                ...
                Self {
                    ...
                    status: Some(workflow_activation_completion::Status::Failed(
                        workflow_completion::Failure {
                            ...
                            force_cause: cause.unwrap_or(WorkflowTaskFailedCause::Unspecified)
                                as i32,
                        },
                    )),
                }
            ...
            pub fn add_internal_flags(&mut self, patch: u32) {
                if let Some(workflow_activation_completion::Status::Successful(s)) =
                    &mut self.status
                {
                ...
                Self {
                    ...
                    variant: Some(activity_task::activity_task::Variant::Start(
                        activity_task::Start {
                            ...
                            workflow_type: r
                                .workflow_type
                                .map_or_else(|| "".to_string(), |wt| wt.name),
                            ...
                            activity_type: r
                                .activity_type
                                .map_or_else(|| "".to_string(), |at| at.name),
        ...
        impl Failure {
            pub fn is_timeout(
                &self,
            ) -> Option<crate::protos::temporal::api::enums::v1::TimeoutType> {
        ...
        pub(super) fn fix_retry_policy(mut retry_policy: RetryPolicy) -> RetryPolicy {
        ...
        mod sdk_helpers {
            use crate::protos::{
                ...
                coresdk::{
                    ...
                    external_data::{LocalActivityMarkerData, PatchedMarkerData},
                },
            ...
            pub fn decode_change_marker_details(
                ...
                if let Some(cd) = details.get(PATCHED_MARKER_DETAILS_KEY) {
                    let decoded =
                        PatchedMarkerData::from_json_payload(cd.payloads.first()?).ok()?;
        ...
        mod sdk_helpers {
            ...
            impl QueryResult {
                ...
                pub fn into_components(
                    self,
                ) -> (String, QueryResultType, Option<Payloads>, String) {
                    match self {
                        QueryResult {
...
pub mod temporal {
    pub mod api {
                    ...
                    impl From<command::Attributes> for Command {
                        fn from(c: command::Attributes) -> Self {
                            match c {
                                ...
                                a @ Attributes::CompleteWorkflowExecutionCommandAttributes(_) => {
                                    Self {
                                ...
                                a @ Attributes::RequestCancelActivityTaskCommandAttributes(_) => {
                                    Self {
                                ...
                                a @ Attributes::CancelWorkflowExecutionCommandAttributes(_) => {
                                    Self {
                                ...
                                a @ Attributes::RequestCancelNexusOperationCommandAttributes(_) => {
                                    Self {
                                        command_type: CommandType::RequestCancelNexusOperation
                                            as i32,
                                        attributes: Some(a),
                                        ..Default::default()
                                    }
        ...
        pub mod cloud {
            ...
            pub mod auditlog {
                pub mod v1 {
                    tonic::include_proto!("temporal.api.cloud.auditlog.v1");
                }
            }
            pub mod billing {
                pub mod v1 {
                ...
                mod sdk_helpers {
                        ...
                        T: AsRef<[u8]>,
                    {
                        fn from(v: T) -> Self {
                            ...
                            let mut metadata = HashMap::new();
                            metadata
                                .insert(ENCODING_PAYLOAD_KEY.to_string(), b"binary/plain".to_vec());
                            Self {
                                metadata,
            ...
            pub mod v1 {
                ...
                mod sdk_helpers {
                    ...
                    pub fn workflow_event_link_from_nexus(
                        ...
                        let run_id = path_parts.get(5).ok_or_else(|| {
                            anyhow!("Failed to parse nexus link, no run id: {:?}", l)
                        })?;
                        ...
                        Ok(common::v1::Link {
                            variant: Some(common::v1::link::Variant::WorkflowEvent(
                                WorkflowEvent {
            ...
            pub mod v1 {
                ...
                mod sdk_helpers {
                    ...
                    impl QueryWorkflowResponse {
                        ...
                        pub fn unwrap(
                            self,
                        ) -> Vec<crate::protos::temporal::api::common::v1::Payload>
                        {
...
mod sdk_helpers {
    ...
    pub fn proto_ts_to_system_time(ts: &prost_types::Timestamp) -> Option<std::time::SystemTime> {
        std::time::SystemTime::UNIX_EPOCH.checked_add(
            Duration::from_secs(ts.seconds as u64) + Duration::from_nanos(ts.nanos as u64),
        )
    }
